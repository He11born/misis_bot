...
def parse_csv_data(csv_content: str) -> bool:
    ...
    try:
        ...
        if len(csv_lines) < 2:
            # Сюда попадем, если есть только заголовок или файл пуст
            logger.warning(f"❌ CSV-содержимое содержит менее двух строк (только {len(csv_lines)}). Данные не загружены.")
            return False

        # --- КРИТИЧЕСКИЙ ФИКС 3: Манипуляция с заголовком и передача DictReader только данных ---
        ...
        reader = csv.DictReader(data_lines, fieldnames=fieldnames, delimiter=delimiter_char)
